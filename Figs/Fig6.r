...
#setwd('/users/noaa/glider/2021/8aug/aug31/rmarkdown/')
#path.dat <- '/users/noaa/glider/2021/8aug/aug25/rmarkdown/tables/NASC_yrs/'
#path.write <- '/users/noaa/glider/2021/9sep/sep7/'

Fig6 <- function(depths = c(150), AMLR.area = 'SA', max.z.b = 50, NASC.yrs = c(2001:2009,2011), n.reps = 9, n.gldr = c(1) ){
  load('NASC_leg1.RData')
  NASC.leg1 <- as.data.frame(NASC.leg1)
  leg1 <- read.csv('leg1.csv')
  leg1 <- leg1[leg1$area == 'SA' | leg1$area =='WA',]
  leg1 <- na.omit(leg1)
  ...
  for (iyr in 1:length(NASC.yrs)){
    ship.NASC[[iyr]] <- read.csv(paste("tables/NASC_yrs/",
      AMLR.area,"_",NASC.yrs[iyr],"_NASC_DAT.csv",sep=''))
  ...
  for(i.gldr in 1:n.gldr){
    ...
    for(iyr in 1:length(NASC.yrs)){
     ...
     tmp <- read.table(paste(cgldr.pth,NASC.yrs[iyr],"_",
               depths,"_cgldrs.txt",sep=""),header=TRUE)
        ...
        smpl.pths <- read.table(paste(s.pth,NASC.yrs[iyr],"_",
