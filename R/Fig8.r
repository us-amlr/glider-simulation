# from C:\zot\glider\2021\2feb\feb3_coverage
...
for(i.area in 1: length(AMLR.area)){
  ship.stats <- read.table(paste('tables/shiptable.txt',sep=""),header=TRUE)
  ...
  for(i.depth in 1:length(depths)){  
    ...
    # tests different glider years for coverage of 68% for the SE
    # gldr.rng are the yo lengths to be tested for coverage, 
    # from R_6_BS_CS.txt
    ...
    write.table(coverage.p,paste("coverage_p_",
  ...
  combo.table <- read.table(paste(combo.files[1],sep=""))
  for (i in 2: length(combo.files))
    combo.table <- cbind(combo.table,read.table(paste(combo.files[i],sep="")))
  ...
  write.table(round(combo.table,2),paste("yo_coverage_table_",depths[i.depth],"m.txt",sep=""))
