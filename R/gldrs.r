...
for(i.pth in 1:length(n.gldr)) # i.pth indexes the number of gliders ("paths") that are sampling
for(i.depth in 1:length(depths)){
  ...
  for(iyr in 1:length(NASC.yrs)){
    ...
    # Calculate gldr.dat, the vector of (NASC) bin values that were sampled.
      ...
      for(i.rep in 1:n.rep){
        #set.seed()
        ifelse(smpl.st > 0, # smpl.st = 1 reads gldr starting location from file
                     {
                      gldr.strt[iyr,] <-  unlist(as.numeric(read.table(paste(strt.path,
                                     'gldr_strt_',NASC.yrs[iyr],"_",depths[i.depth],"m.txt",sep=""),
                                      header = TRUE)))
                     },
        ...
        ifelse(n.gldr == 1,gldr.strt.table[,i.rep] <- gldr.strt,
               gldr.strt.table[i.pth,i.rep] <- gldr.strt[i.pth]))
     ...
     #   if glider is going below 500 m:
      ...
      #####################
      write.table(smpl.pths.multi,paste('tables/smpl_paths/',n.gldr[i.pth],"_",
                                 AMLR.area,'_',smpl.yr,"_",depths[i.depth],"_spths.txt",sep="")) 
      } # end of i.rep loop

    write.table(round(c.gldrs,5),paste('tables/cgldrs/',n.gldr[i.pth],"_",
    ...
    if(save.tables == 1){
      write.table(round(gldr.table,digits=1),
        file=paste('tables/gldrtables/',n.gldr[i.pth],"_",
         AMLR.area,'_',smpl.yr,'_',depths[i.depth],"m_gldrtable.txt",sep=""))
      write.table(gldr.strt.table,paste("tables/gldr_strt/",
         n.gldr[i.pth],'_', AMLR.area,"_gldr_strt_",NASC.yrs[iyr],"_",depths[i.depth],"m.txt",sep=""))
      write.table(round(yo.sums,5),paste("tables/yo_sums/",
         n.gldr[i.pth],'_', AMLR.area,"_",smpl.yr,"_",depths[i.depth],"_yo_sums.txt",sep=""))
      write.csv(round(t(NASC.dat),4),
         file=paste('tables/NASC_yrs/',n.gldr[i.pth],"_",AMLR.area,"_",smpl.yr,"_NASC_dat.csv",sep=""))
      }
      #####################
    } # end iyr loop
  ...
  if(save.tables == 1){
    write(ship.Sv,"tables/ship_Sv.txt",ncolumns=10)
    write.table(ship.table,row.names=FALSE,
        file=paste("tables/shiptable.txt",sep=""))
    write.table(round(gldr.sums,5),paste("tables/gldr_stats/",
        n.gldr,'_', AMLR.area,"_",depths[i.depth],"_gldr_sums.txt",sep=""))
    write.table(round(gldr.sds,5),paste('tables/gldr_stats/',
        n.gldr,'_', AMLR.area,"_",depths[i.depth],"_gldr_sd.txt",sep=""))
    write(yo.count*n.gldr[i.pth],paste("tables/",
        n.gldr,'_', AMLR.area,'_',depths[i.depth],"_yo_count.txt",sep=""),ncolumns=10)
  } # end save.tables
